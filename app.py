        ...
        response["data"].append(
            {
                ...
                "num_upcoming_shows": len(
                    Show.query.filter(
                        Show.venue_id == venue.id and Show.start_time > datetime.now()
                    ).all()
                ),
            }
        )
...
def show_venue(venue_id):
    ...
    try:
        ...
        def past_shows_fxn(venue_id):
            shows_query = (
                db.session.query(Show).join(Artist).filter(Show.venue_id == venue_id)
            )
                    ...
                    data[0]["past_shows"].append(
                        {
                            ...
                            "start_time": shows.start_time.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
                        }
                    )
...
def show_artist(artist_id):
    ...
    data["genres"] = artist_information.genres
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        edit_current_venue.name = request.form.get("name")
        edit_current_venue.city = request.form.get("city")
        edit_current_venue.state = request.form.get("state")
        edit_current_venue.address = request.form.get("address")
        edit_current_venue.phone = request.form.get("phone")
        edit_current_venue.genres = request.form.getlist("genres")
        ...
        data.append(
            {
                ...
                "upcoming_shows": len(
                    Show.query.filter_by(artist_id=all.id)
        ...
        data.append(
            {
                ...
                "upcoming_shows": len(
                    Show.query.filter_by(venue_id=all.id)
                    .filter(Show.start_time > datetime.now())
                    .all()
                ),
            }
...
def create_show_submission():
    ...
    try:
        ...
        if isArtist is None and isVenue is None:
            flash(
                "venue and artist dose not exist, create an artist profile and venue profile first"
            )
            return render_template("pages/home.html")
